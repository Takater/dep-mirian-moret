import React from 'react'

function Quem () {
    return (
        <div id="quemsoueu">
            <div className="row">
                <div className="col-md-12">
                    <div className="main-timeline5">
                        <div className="timeline">
                            <div className="timeline-icon"><span class="year">1966</span></div>
                            <div className="timeline-content">
                                <h3 className="title">Quem sou eu?</h3>
                                <p className="description">Me chamo Mirian Moret de Freitas Lucena, tenho 56 anos, sou natural do
                                Rio de Janeiro. Casada há 27 anos com Rogélio Pinheiro Lucena (em memória), tenho 4 lindos filhos.
                                Filha do Pastor Joil Dias de Freitas e Noemia Moret de Freitas.</p>
                            </div>
                        </div>
                        <div className="timeline">
                            <div className="timeline-icon"><span class="year">1983</span></div>
                            <div className="timeline-content">
                                <h3 className="title">Minha história em Rondônia</h3>
                                <p className="description">Me mudei do Paraná para Rondônia em 1983 com meus pais, onde meu pai veio para
                                assumir a presidência da Primeira Igreja Batista. Em 1990 fui aprovada no concurso de agente penitenciário,
                                desde então, atuo no serviço público, assumindo cargos de gerência e diretoria.</p>
                            </div>
                        </div>
                        <div className="timeline">
                            <div className="timeline-icon"><span class="year">1996</span></div>
                            <div className="timeline-content">
                                <h3 className="title">Meu currículo</h3>
                                <p className="description">
                                    <ul style={{listStyleType: 'circle'}}>
                                        <li>Em 1996, assumi a direção da creche do presídio feminino - SUPEN (RO)</li>
                                        <li>Em 1997, assumi a direção do grupo de recursos humanos - SUPEN (RO)</li>
                                        <li>Em 1998, assumi a direção do albergue feminino - SUPEN (RO)</li>
                                        <li>Entre 1999 a 2003, fui diretora da casa da adolescente - FASER</li>
                                        <li>Ainda em 2003, assumi a direção da casa do adolescente e internação provisória - FASER</li>
                                    </ul>
                                </p>
                            </div>
                        </div>
                        <div className="timeline">
                            <div className="timeline-icon"><span class="year">2004</span></div>
                            <div className="timeline-content">
                                <h3 className="title">Meu currículo</h3>
                                <p className="description">
                                    <ul style={{listStyleType: 'circle'}}>
                                        <li>Em 2004, assumi a presidência da empresa municipal de transportes urbanos de
                                            Ji-Paraná - EMTU</li>
                                        <li>Também em 2004, em conjunto, assumi a Secretaria Municipal de Ação Social -
                                            SEMAS (Ji-Paraná)</li>
                                        <li>De 2005 a 2007, gerenciei o atendimento a criança e ao adolescente - FASER</li>
                                        <li>De 2008 a 2018, fui chefe do setor de visita da penitenciária Estadual Ênio
                                            dos Santos Pinheiro (RO)
                                        </li>
                                        <li>Em 2018, assumi a gerência de Saúde da Secretaria do Estado da Justiça</li>
                                        <li>De 2019 a 2022, lotada na penitenciária feminina de Porto Velho</li>
                                    </ul>
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Quem;